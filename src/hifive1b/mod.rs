...
impl AddrBus for Memory {
    ...
    fn read_byte(&self, addr: usize) -> anyhow::Result<u32> {
        ...
        if self.is_uart0(addr) {
            return Ok(u32::from(self.uart0.read_byte(addr - self.uart0_base)));
        }
        if self.is_uart1(addr) {
            return Ok(u32::from(self.uart1.read_byte(addr - self.uart1_base)));
        }
        if self.is_clic(addr) {
            return match addr {
                ...
                0x200_BFF8..=0x0200_BFFF => Ok(self.timer.read_byte(addr - 0x200_BFF8).into()), // mtime Timer register
                _ => Err(anyhow::anyhow!(
    ...
    fn write_byte(&mut self, addr: usize, value: u32) -> anyhow::Result<()> {
        ...
        if self.is_uart0(addr) {
            self.uart0
                .write_byte(addr - self.uart0_base, (value & 0xFF) as u8);
            return Ok(());
        }
        if self.is_uart1(addr) {
            self.uart1
                .write_byte(addr - self.uart1_base, (value & 0xFF) as u8);
            return Ok(());
        }
        if self.is_clic(addr) {
            return match addr {
                ...
                0x200_BFF8..=0x0200_BFFF => {
                    self.timer.write_byte(addr - 0x200_BFF8, value as u8);
                    Ok(()) // mtime Timer register
                }
                _ => Err(anyhow::anyhow!(
        ...
        if self.is_clic(addr) {
            return match addr {
                ...
                0x200_4000..=0x0200_4003 => {
                    // mtimecmp for hart 0 MTIMECMP Registers, lower half
                    self.timer.set_timecmp(value.into());
                    Ok(())
                }
                0x200_4004..=0x0200_4007 => {
                    // mtimecmp for hart 0 MTIMECMP Registers, upper half
                    Ok(())
                }
                0x200_BFF8..=0x0200_BFFB => {
                    // mtime Timer register, lower half
                    self.timer.write_word(addr - 0x200_BFF8, value);
                    Ok(())
                }
                0x200_BFFC..=0x0200_BFFF => {
                    // mtime Timer register, upper half
                    Ok(())
                }
                _ => Err(anyhow::anyhow!(
                    "Clic: attempted write outside memory map at address: 0x{addr:08X}"
